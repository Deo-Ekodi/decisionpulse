...
object MockRepository {
    ...
    val insights: List<AiInsight> = listOf(
        AiInsight(
            id = 1, farmCode = "NYR-019",
            title = "Declining Yield — Intervention Needed",
            body = "NYR-019 has dropped 7.8% over 14 days. Pattern matches farms in Mathira East that responded to Napier fodder supplementation in Q3 last year.",
            action = "Schedule extension visit. Review feed sourcing.",
            priority = InsightPriority.HIGH, type = InsightType.ALERT, hoursAgo = 2
        ),
        AiInsight(
            id = 2, farmCode = "NYR-044",
            title = "Feed Cost 108% Above SACCO Average",
            body = "Cost per litre at KES 25.20 vs. SACCO average of KES 15.41. Main driver appears to be concentrate overuse without proportional yield improvement.",
            action = "Audit feed mix. Compare with NYR-038 input schedule.",
            priority = InsightPriority.HIGH, type = InsightType.COST, hoursAgo = 4
        ),
        AiInsight(
            id = 3, farmCode = null,
            title = "7 Farms Within 20L of 100L Target",
            body = "At current growth rates, NYR-001, NYR-023, NYR-051, and NYR-062 will reach 100L/day before end of Q2 2026. Recognition at next AGM recommended.",
            action = "Prepare performance certificates. Use as peer motivation.",
            priority = InsightPriority.MEDIUM, type = InsightType.OPPORTUNITY, hoursAgo = 8
        ),
        AiInsight(
            id = 4, farmCode = "NYR-038",
            title = "Top Performer Benchmark Available",
            body = "NYR-038 runs at 95.8L/day with KES 10.80/L — lowest cost in the SACCO. Their Brachiaria grass rotation and 4-month feed buffer model is replicable.",
            action = "Document NYR-038 practices. Share at next SACCO meeting.",
            priority = InsightPriority.MEDIUM, type = InsightType.OPPORTUNITY, hoursAgo = 12
        ),
        AiInsight(
            id = 5, farmCode = null,
            title = "Feed Logging Drives 18% Better Yield",
            body = "Farms logging feed costs weekly outperform non-loggers by an average of 18.3% on yield. The discipline of tracking appears to directly change daily decisions.",
            action = "Encourage all farms to log weekly. A simple WhatsApp report is enough.",
            priority = InsightPriority.MEDIUM, type = InsightType.PERFORMANCE, hoursAgo = 20
        ),
        AiInsight(
            id = 6, farmCode = "NYR-012",
            title = "Consistency Score Recovering",
            body = "NYR-012 dropped to 38 in January but recovered to 52 after the coordinator visit in February. Still below target — trend is positive.",
            action = "Continue monthly coordinator check-ins. No emergency intervention needed.",
            priority = InsightPriority.LOW, type = InsightType.PERFORMANCE, hoursAgo = 36
        ),
    )

    fun getFarmByCode(code: String): Farm? =
        farms.find { it.code == code }
