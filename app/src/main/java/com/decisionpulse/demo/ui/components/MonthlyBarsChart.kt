...
fun MonthlyBarsChart(
    ...
    Canvas(modifier = modifier) {
        ...
        data.forEachIndexed { i, v ->
            ...
            val color      = if (highlightLast && i == count - 1) barColor else barColor.copy(alpha = 0.55f)

            drawRoundRect(color = trackColor, topLeft = Offset(x, 0f), size = Size(barWidth, size.height), cornerRadius = radius)
            if (fillHeight > 0f)
                drawRoundRect(color = color, topLeft = Offset(x, size.height - fillHeight), size = Size(barWidth, fillHeight), cornerRadius = radius)
        }
    }
}